...
defsym_expr:
		...
		{
		  ldlex_popstate();
                    printf("ldgram: lang_add_assignment (exp_defsym (%x, %x)) \n", $2, $4);
...
mri_script_command:
	...
	|	INCLUDE filename
		{ ldlex_script (); printf("ldgram: dfsdasdfsaf \n"); ldfile_open_command_file($2);}
...
script_file:
	{ ldlex_both(); printf("ldgram: ldlex_both() \n");}
	ifile_list
	{ ldlex_popstate(); }
        ;
...
ifile_p1:
	...
	|	GROUP
		...
		{ lang_add_map($3); }
	|	INCLUDE filename
		{ ldlex_script (); printf("ldram: INCLUDE filename \n"); ldfile_open_command_file($2); }
		ifile_list END
		{ ldlex_popstate (); }
...
statement_anywhere:
		ENTRY '(' NAME ')'
		{ lang_add_entry ($3, FALSE); printf("ldgram:lang_add_entry %s \n", $3); }
	|	assignment end
	|	ASSERT_K  {printf("ldgram: ASSERT_K\n"); ldlex_expression ();} '(' exp ',' NAME ')'
		{ ldlex_popstate ();
...
wildcard_spec:
	...
	| 	EXCLUDE_FILE '(' exclude_name_list ')' wildcard_name
			{
			  printf("EXCLUDE_FILE '(' exclude_name_list ')'\n");
			  $$.name = $5;
			  $$.sorted = none;
			  $$.exclude_name_list = $3;
			  $$.section_flag_list = NULL;
	...
	|	SORT_BY_NAME '(' wildcard_name ')'
			{
			  printf("ldgram: SORT_BY_NAME '(' wildcard_name ')'\n");
			  $$.name = $3;
			  $$.sorted = by_name;
			  $$.exclude_name_list = NULL;
			  $$.section_flag_list = NULL;
	...
	|	SORT_BY_ALIGNMENT '(' wildcard_name ')'
			{
			  printf("SORT_BY_ALIGNMENT '(' wildcard_name ')' \n");
			  $$.name = $3;
			  $$.sorted = by_alignment;
			  $$.exclude_name_list = NULL;
			  $$.section_flag_list = NULL;
...
sect_flag_list:	NAME
			{
              printf("ldgram:sect_flag_list: %s %s \n", $1[0], $1[1]);
...
sect_flags:
		INPUT_SECTION_FLAGS '(' sect_flag_list ')'
			{
              printf("INPUT_SECTION_FLAGS '(' sect_flag_list ')' \n");
			  struct flag_info *n;
		...
		NAME
			{
			  ...
			  tmp.name = $1; printf("ldgram: tmp.name %d", $1);
...
statement:
	...
	| input_section_spec
        | length '(' mustbe_exp ')'
			  ...
			  printf("ldgram: length '(' mustbe_exp ), %s, %s \n", $1, $3);
			}
	...
	| ASSERT_K  {ldlex_expression ();} '(' exp ',' NAME ')' end
			{ ldlex_popstate ();
			  lang_add_assignment (exp_assert ($4, $6)); }
	| INCLUDE filename
		{ printf("ldgram: include %s \n", $2);
		 ldlex_script (); ldfile_open_command_file($2); printf("ldgram: cccc\n");}
	...
	mustbe_exp
		{
          ...
          printf("ldgram: $$ = exp_get_fill ($1, 0, fill value) \n");
		}
	;
...
assignment:
		NAME '=' mustbe_exp
		{
          printf("ldgram: NAME '=' mustbe_exp ==> %d, %x\n", *$1, *$3);
		  lang_add_assignment (exp_assign ($1, $3, FALSE));
		}
	|	NAME assign_op mustbe_exp
		{
		  printf("NAME assign_op mustbe_exp \n");
		  lang_add_assignment (exp_assign ($1,exp_binop ($2, exp_nameop (NAME, $1),$3), FALSE));
		}
	|	HIDDEN '(' NAME '=' mustbe_exp ')'
		{
		  printf("NHIDDEN '(' NAME '=' mustbe_exp ')'\n");
		  lang_add_assignment (exp_assign ($3, $5, TRUE));
		}
	|	PROVIDE '(' NAME '=' mustbe_exp ')'
		{
		  printf("PROVIDE '(' NAME '=' mustbe_exp ')' \n");
		  lang_add_assignment (exp_provide ($3, $5, FALSE));
		}
	|	PROVIDE_HIDDEN '(' NAME '=' mustbe_exp ')'
		{
		  printf("PROVIDE_HIDDEN '(' NAME '=' mustbe_exp ')' \n");
...
memory_spec: 	NAME
		{printf("ldgram: region = lang_memory_region_lookup ==> %s\n", $1); region = lang_memory_region_lookup ($1, TRUE);}
		attributes_opt ':'
		origin_spec opt_comma length_spec
		{}
	|	INCLUDE filename
		{ ldlex_script (); ldfile_open_command_file($2); printf("ldgram: ldfile_open_command_file($2, %d)\n", $2);}
...
attributes_string:
		NAME
		  { printf("ldgram: attributes_string ==> %s\n", $1); lang_set_flags (region, $1, 0); }
	|	'!' NAME
		  { printf("ldgram: attributes_string 2 ==> !%s\n", $2); lang_set_flags (region, $2, 1); }
	;

startup:
	STARTUP '(' filename ')'
		{  printf("ldgram: lang_startup($3);n"); lang_startup($3); }
	;
...
mustbe_exp: { ldlex_expression (); }
		exp
			 {printf("ldgram: ldlex_expression() ==> %x \n", $2); ldlex_popstate (); $$=$2; }
	;
...
exp	:
	...
	|	NEXT '(' exp ')' %prec UNARY
			{ $$ = exp_unop ((int) $1,$3); }
	|	'!' exp %prec UNARY
			{ $$ = exp_unop ('!', $2); printf("ldgram: exp_unop ('!', %s); \n", $2); }
	|	'+' exp %prec UNARY
			{ $$ = $2; }
	|	'~' exp %prec UNARY
			{ $$ = exp_unop ('~', $2);}
	...
	|	exp '*' exp
			{ $$ = exp_binop ('*', $1, $3); printf("ldgram:  exp_binop ('*', %s, %s); \n", $1, $3); }
	|	exp '/' exp
			{ $$ = exp_binop ('/', $1, $3); }
	|	exp '%' exp
			{ $$ = exp_binop ('%', $1, $3); }
	...
	|	ABSOLUTE '(' exp ')'
			{ $$ = exp_unop (ABSOLUTE, $3); }
	|	ALIGN_K '(' exp ')'
			{ $$ = exp_unop (ALIGN_K,$3); printf("ldgram: exp_unop (ALIGN_K,$3); \n");}
	|	ALIGN_K '(' exp ',' exp ')'
	...
	|	ORIGIN '(' NAME ')'
			{ $$ = exp_nameop (ORIGIN, $3); }
	|	LENGTH '(' NAME ')'
			{ printf("ldgram: LENGTH %s \n", $3); $$ = exp_nameop (LENGTH, $3); }
	|	LOG2CEIL '(' exp ')'
			{ $$ = exp_unop (LOG2CEIL, $3); }
	;
...
memspec_at_opt:
                AT '>' NAME { $$ = $3; printf("ldgram: AT '>' NAME \n");}
        |       { $$ = 0; }
        ;

opt_at:
		AT '(' exp ')' { $$ = $3; printf("ldgram: AT '(' exp ')'\n");}
	|	{ $$ = 0; }
	;

opt_align:
		ALIGN_K '(' exp ')' { $$ = $3; printf("ldgram: ALIGN_K '(' exp ')'\n");}
	|	{ $$ = 0; }
	;

opt_align_with_input:
		ALIGN_WITH_INPUT { $$ = ALIGN_WITH_INPUT; printf("ldgram: opt_align_with_input\n");}
	|	{ $$ = 0; }
	;
...
section:	NAME 		{ ldlex_expression(); printf("ldgram: section -> name -> ldlex_expression()\n");}
		opt_exp_with_type
		opt_at
		opt_align
		opt_align_with_input
		...
		'{'
			{
                          printf("ldgram: lang_enter_output_section_statement %s, %s, %s, %s, %s, %s, %s\n", $1, $3, $5, $7, $4, $9, $6);
	...
	|	/* The GROUP case is just enough to support the gcc
		...
		'{' sec_or_group_p1 '}'
	|	INCLUDE filename
		{ ldlex_script (); ldfile_open_command_file($2); printf("ldgram: ldfile_open_command_file($2) 1\n");}
		sec_or_group_p1 END
		{ ldlex_popstate (); }
	;
...
opt_exp_without_type:
		exp ':'		{ $$ = $1; printf("ldgram: opt_exp_without_type \n");}
	|	':'		{ $$ = (etree_type *) NULL;  }
	;
...
vers_defns:
		VERS_IDENTIFIER
		{
          printf("ldgram: lang_new_vers_pattern %s \n", $1);
